# AgroManagement Autotests

End-to-end QA automation project for **AgroManagement** with a dedicated Playwright + TypeScript framework and CI/CD pipeline.
...
- `AgroManagement/` - Spring Boot application under test (backend + Thymeleaf UI)
- `test-automation-framework/` - Playwright + TypeScript test framework

Additional artifacts:
- `AGROMANAGEMENT_DEEP_QA_ANALYSIS.md` - deep system QA analysis
- `AGROMANAGEMENT_TEST_FRAMEWORK_ARCHITECTURE.md` - framework architecture/design
...
- `api/clients` - API client layer
- `api/services` - API service abstractions
...
- `scripts/cleanupTestData.ts` - cleanup strategy
...
2. Starts `AgroManagement` stack with Docker Compose
